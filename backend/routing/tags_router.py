...
async def create_tag(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Ошибка при создании тега"
        )


@router.get("/", response_model=List[TagResponse], summary="Получение всех тегов")
...
async def get_tags_with_count(repo: TagsRepository = Depends(get_tags_repository)):
    """Получить теги с количеством тренингов"""
    tags_with_count = await repo.get_with_trainings_count()

    return [
        TagWithTrainingsCount(value=tag.value, label=tag.label, trainings_count=count)
        for tag, count in tags_with_count
    ]


@router.get("/{tag_value}", response_model=TagResponse, summary="Получение тега по ID")
async def get_tag_by_id(
    tag_value: int, repo: TagsRepository = Depends(get_tags_repository)
):
...
@router.patch("/{tag_value}", response_model=TagResponse, summary="Обновление тега")
async def update_tag(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Ошибка при обновлении тега"
        )


@router.delete(
    "/{tag_value}", status_code=status.HTTP_204_NO_CONTENT, summary="Удаление тега"
)
